...
from models import KGEModel, Rotate3D, RotatE, Rotate4D_v1, Rotate4D_v2
from data import TrainDataset, BatchType, ModeType, DataReader
from data import BidirectionalOneShotIterator

def type_relation(triples):
    cnt_h, cnt_r, cnt_t = {}, {}, {}
    for head, relation, tail in triples:
        if relation not in cnt_r:
            cnt_r[relation] = 0
            cnt_h[relation], cnt_t[relation] = set(), set()
        cnt_r[relation] += 1
        cnt_h[relation].add(head)
        cnt_t[relation].add(tail)

    rel_type = {}
    for r in range(len(cnt_r)):
        tph = cnt_r[r] / len(cnt_h[r]) # tail per head
        hpt = cnt_r[r] / len(cnt_t[r]) # head per tail
        if hpt < 1.5:
            if tph < 1.5:
                rel_type[r] = 1  # 1-1
            else:
...
def main(args):
    ...
    elif args.model == 'Rotate4D_v1':
        kge_model = Rotate4D_v1(num_entity, num_relation, args.hidden_dim, args.gamma, args.p_norm, dataset)
    elif args.model == 'Rotate4D_v2':
        kge_model = Rotate4D_v2(num_entity, num_relation, args.hidden_dim, args.gamma, args.p_norm, dataset)
    else:
    ...
    rel_type = type_relation(TrainDataset(data_reader, args.negative_sample_size, BatchType.HEAD_BATCH).triples)
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if args.do_valid and step % args.valid_steps == 0:
                ...
                metrics, metrics_rel, metrics_h, metrics_t = kge_model.module.test_step(kge_model, data_reader, ModeType.VALID, args, rel_type)
                log_metrics('Valid', step, metrics)

            if args.do_test and step % args.valid_steps == 0:
                logging.info('Evaluating on Test Dataset...')
                metrics, metrics_rel, metrics_h, metrics_t = kge_model.module.test_step(kge_model, data_reader, ModeType.TEST, args, rel_type)

                logging.info("----------------Overall Results----------------")
                log_metrics('Test', step, metrics)

                logging.info("----------------Head Prediction----------------")
                logging.info("----------------------1-1----------------------")
                log_metrics('Test', step, metrics_h[1])
                logging.info("----------------------1-N----------------------")
                log_metrics('Test', step, metrics_h[2])
                logging.info("----------------------N-1----------------------")
                log_metrics('Test', step, metrics_h[3])
                logging.info("----------------------N-N----------------------")
                log_metrics('Test', step, metrics_h[4])

                logging.info("----------------Tail Prediction----------------")
                logging.info("----------------------1-1----------------------")
                log_metrics('Test', step, metrics_t[1])
                logging.info("----------------------1-N----------------------")
                log_metrics('Test', step, metrics_t[2])
                logging.info("----------------------N-1----------------------")
                log_metrics('Test', step, metrics_t[3])
                logging.info("----------------------N-N----------------------")
                log_metrics('Test', step, metrics_t[4])

                logging.info("-----------Result for each Relation------------")
    ...
    if args.evaluate_train:
        ...
        metrics, metrics_rel, metrics_h, metrics_t = kge_model.test_step(kge_model, data_reader, ModeType.TRAIN, args, rel_type)
