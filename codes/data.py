    ...
    def __getitem__(self, idx: int):
        ...
        while neg_size < self.neg_size:
            ...
            if self.batch_type == BatchType.HEAD_BATCH:
                mask = np.in1d(neg_triples_tmp, self.tr_map[(tail, rel)], assume_unique=True, invert=True)
            elif self.batch_type == BatchType.TAIL_BATCH:
                mask = np.in1d(neg_triples_tmp, self.hr_map[(head, rel)], assume_unique=True, invert=True)
            else:
...
class TestDataset(Dataset):
    def __init__(self, data_reader: DataReader, mode: ModeType, batch_type: BatchType, rel_type: dict):
    ...
    def __getitem__(self, idx):
        ...
        return positive_sample, negative_sample, filter_bias, self.batch_type, torch.tensor([self.rel_type[relation]])
    ...
    def collate_fn(data):
        ...
        mode = data[0][3]
        rel_type = torch.cat([_[4] for _ in data], dim=0)
        return positive_sample, negative_sample, filter_bias, mode, rel_type
